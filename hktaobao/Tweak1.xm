            ...
            [arg1 enumerateObjectsUsingBlock:^(id cell, NSUInteger idx, BOOL *stop) {
                ...
                if([[cell getBizType] isEqualToString:@"auction"]){
                    if ([cell isKindOfClass:NSClassFromString(@"TBSearchResultItemTips")]) {
                        ...
                        //查找宝贝
                        NSLog(@"TBHK: Main from id ==> %@, title: %@,clickUrl:%@  %d, %@", [cell item_id], [cell title],[cell clickUrl], (int)[self currentPage], [[cell item_id] isEqualToString:[itemInfo objectForKey:@"id"]] ? @"YES": @"NO");
                        ...
                        if([[itemInfo objectForKey:@"dmIsTrainMust"] intValue] == 1){
                            ...
                            if ([[cell item_id] isEqualToString:[itemInfo objectForKey:@"id"]]) {
                                findItem = YES;
                                myItem = cell;
                            }
                        }
...
- (void)requestSuccessWithResult:(id)arg1 {
    ...
    NSArray *modelNameList = @[@"TBOrderAddressModel", @"TBOrderInfoModel", @"TBOrderPayDetailModel", @"TBOrderSellerModel", @"TBOrderSubModel", @"TBOrderMemoModel"];
