...
- (id)init{
    ...
    dispatch_group_async(group, queue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            //            NSLog(@"m_nsAliasName:%@ m_nsNickName:%@ m_nsMobile:%@ m_nsUsrName:%@ m_uiSex:%d",[self m_nsAliasName],[self m_nsNickName],[self m_nsMobile],[self m_nsUsrName],[self m_uiSex]);

            //写入文件
            NSString *userInfo = [NSString stringWithFormat:@"{\"nsAliasName\":\"%@\",\"nsNickName\":\"%@\",\"nsMobile\":\"%@\",\"nsUsrName\":\"%@\",\"uiSex\":\"%d\"}",[self m_nsAliasName],[self m_nsNickName],[self m_nsMobile],[self m_nsUsrName],[self m_uiSex]];
            NSLog(@"hkwxChecnAccount %@",userInfo);
            write2File(@"/var/root/checkAccount/userInfo.json", userInfo);
        });
