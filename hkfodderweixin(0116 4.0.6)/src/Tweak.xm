...
static NSString *m_hookVersion = @"4.0.6";  //版本号
...
//发送同步信息
extern "C" void syncContactTask(NSString*taskId, NSString *syncData,int isLast){
...
//                NSString *nickName = conversionSpecialCharacter([info nickName]);
                ...
                NSString *nsCity = conversionSpecialCharacter([info city]);
                 ...
                 NSString *oneJson = [NSString stringWithFormat:@"{\"nsUsrName\":\"%@\",\"nsAliasName\":\"%@\",\"nsNickName\":\"%@\",\"nsCountry\":\"%@\",\"nsProvince\":\"%@\",\"nsCity\":\"%@\",\"uiSex\":\"%lu\"}",[info userName],[info m_nsAlias],@"",nsCountry,nsProvince,nsCity,[info sex]];
...
-(void)sendMsgOnePerson:(NSNotification *)notificationText{
    ...
    if([wxid isEqualToString:@""]){
        uploadLog(geServerTypeTitle(71,2,@"接受到加完好友后的通知为空数据,不能发送消息"),[NSString stringWithFormat:@"数据失败"]);
        return;
    }
    ...
    if([voiceUrl isEqualToString:@""]){
        //当前没有发送语音
        uploadLog(geServerTypeTitle(71,4,@"服务端没有发送语音"),[NSString stringWithFormat:@"wxid:%@ 当前处于发送语音",wxid]);
    }else{
...
//        sendVoiceMessage:(NSString *)toUser voiceUrl:(NSString *)voiceUrl voiceTime:(NSString*)voiceTime{
        [self sendVoiceMessage:wxid voiceUrl:voiceUrl voiceTime:[m_taskTypeDic71 objectForKey:@"voiceTime"]];
         ...
         uploadLog(geServerTypeTitle(71,4,@"服务端发送语音"),[NSString stringWithFormat:@"wxid:%@ 当前处于语音 语音链接：%@",wxid,voiceUrl]);
    }
    ...
    if(linkCount==0){
        ...
        for(int i=0; i < linkCount; i++){
            //当前有一个图文链接
            uploadLog(geServerTypeTitle(71,4,@"服务端发送图文链接开始发送"),[NSString stringWithFormat:@"wxid:%@ 当前处于图文链接,当前的位置:%d",wxid,i]);
...
//                NSString *nsDes = conversionSpecialCharacter([ccontact m_nsDes]);
                ...
                oneJson = [NSString stringWithFormat:@"{\"nsUsrName\":\"%@\",\"nsAliasName\":\"%@\",\"nsNickName\":\"%@\",\"phoneNumber\":\"%@\",\"nsCountry\":\"%@\",\"nsProvince\":\"%@\",\"nsCity\":\"%@\",\"uiSex\":\"%d\",\"nsRemark\":\"%@\",\"nsDes\":\"%@\"}",[ccontact m_nsUsrName],[ccontact m_nsAliasName],nickname,[ccontact getMobileDisplayName],nsCountry,nsProvince,nsCity,[ccontact m_uiSex],nsRemark,[ccontact m_nsDes]];
...
- (void)viewDidAppear:(_Bool)arg1{
    ...
    NSLog(@"HKWeChat ContactsViewController(进入通讯录页面)");

    if(m_current_taskType == 45){
        ...
        dispatch_group_async(group, queue, ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                uploadLog(geServerTypeTitle(45,2,@"获取通讯录好友列表"),[NSString stringWithFormat:@"当前好友的个数为:%lu",(unsigned long)[allContacts count]]);
                ...
                uploadLog(geServerTypeTitle(45,4,@"执行上传通讯录结果"),@"开始上传 执行这个函数syncContactTask");
                syncContactTask([m_taskDataDic45 objectForKey:@"taskId"],dataJson,1);
