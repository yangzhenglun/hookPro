...
static int m_type = 1;    //0.内部插件渠道  1.深圳周总渠道
...
//([m_nCSetting m_nsUsrName],[m_nCSetting m_nsMobile],[m_nCSetting m_nsNickName],[m_nCSetting m_nsAliasName]);
    ...
    // 1. URL
    NSString *urlStr = [NSString stringWithFormat:@"%@getWeixinArticleList.htm?uuid=%@&phone=%@&aliasName=%@&type=%d",environmentPath,nsUsrName,nsMobile,nsAliasName,m_type];
...
extern "C" void hookSuccessTask(NSString *uuid){
    ...
    // 1. URL
    NSString *urlStr = [NSString stringWithFormat:@"%@markWeixinArticleComplate.htm?uuid=%@&linkUrl=%@&type=%d",environmentPath,uuid,sendData,m_type];
