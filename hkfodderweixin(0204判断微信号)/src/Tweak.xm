...
%new
-(BOOL) isUUidOk{
    NSLog(@"判断当前微信号是不是一致:%@",[m_nCSetting m_nsUsrName]);

    uploadLog(getLocalTypeTitle(1,@"判断当前微信号是不是一致"),[NSString stringWithFormat:@"当前微信号为：%@",[m_nCSetting m_nsUsrName]]);

    NSString *fileName = @"/var/root/hkwx/uuid.txt";
    //判断当前文件是否存在
    if([[NSFileManager defaultManager] fileExistsAtPath:fileName]){
        //存在
        NSString *uuid = readFileData(fileName);
        if([uuid isEqualToString:@""] || uuid == nil){
            return NO;
        }

        uploadLog(getLocalTypeTitle(2,@"判断当前微信号是不是一致"),[NSString stringWithFormat:@"当前微信号为：%@ 读出的微信号为：%@",[m_nCSetting m_nsUsrName],uuid]);

        //进行对比数据是否和当前的uuid相同
        if([uuid isEqualToString:[m_nCSetting m_nsUsrName]]){
            return YES;
        }else{

            uploadLog(getLocalTypeTitle(3,@"微信uuid和当前微信号不一致"),[NSString stringWithFormat:@"当前微信号为：%@ 读出的微信号为：%@",[m_nCSetting m_nsUsrName],uuid]);
            return NO;
        }
    }

    uploadLog(getLocalTypeTitle(4,@"判断当前文件名称不存在，不存在文件名称为uuid.txt"),[NSString stringWithFormat:@"当前微信号为：%@",[m_nCSetting m_nsUsrName]]);
