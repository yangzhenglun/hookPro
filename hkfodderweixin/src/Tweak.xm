...
static NSString *m_hookVersion = @"4.6.4";  //版本号
...
static BOOL m_scanQRCode = NO;         //判断是否已经进入扫码
...
static BOOL m_isEnterRequestHome = NO;  //判断是否请求数据回首页了
...
static int m_frequently_count = 0;          //判断当前频繁的次数
NSMutableArray *m_frequentlyUser = [[NSMutableArray alloc] init]; //频繁的用户
...
//启动时获取京东cookie
extern "C" NSString *getExistsJdCookie(NSString *uuid){
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@existsJdCookie.htm?weixinUuid=%@&pluginKind=0",environmentPath,uuid];
...
//checkNoDuplicateNearbyStrangers([taskDataDic objectForKey:@"taskId"],dataJson,[taskDataDic objectForKey:@"latitude"],[taskDataDic objectForKey:@"longitude"]);
///checkNoDuplicateNearbyStrangers.htm?taskId=&uuid=&phone=&dataList=&longitude=&latitude=
//返回数据：111@stranger,111@stranger,111@stranger      #可以加人的stranger列表
//启动时请求的的任务数据
extern "C" NSString *checkNoDuplicateNearbyStrangers(NSString *taskId,NSString *dataJson,NSString *latitude,NSString *longitude){
...
//    NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@checkNoDuplicateNearbyStrangers.htm",environmentPath];
    //把传进来的URL字符串转变为URL地址
    NSURL *url = [NSURL URLWithString:urlStr];
    ...
    NSString *parseParamsResult = [NSString stringWithFormat:@"encodeType=1&taskId=%@&uuid=%@&phone=%@&dataList=[%@]&longitude=%@&latitude=%@",taskId,[m_nCSetting m_nsUsrName],[m_nCSetting m_nsMobile],dataJson,longitude,latitude];


    NSLog(@"parseParamsResult:%@",parseParamsResult);
...
extern "C" void uploadJdCookie(NSString *weixinUuid,NSString *cookie){
    ...
    NSString *parseParamsResult = [NSString stringWithFormat:@"weixinUuid=%@&cookie=%@&pluginKind=0&hookVersion=0",weixinUuid,cookie];
...
//获取发送阅读的个数
extern "C" void uploadArticleReadNumber(NSString *data,NSString *readNum){
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@uploadClientArticleReadNum.htm",environmentPath];
    //把传进来的URL字符串转变为URL地址
    NSURL *url = [NSURL URLWithString:urlStr];
    ...
    NSString *parseParamsResult = [NSString stringWithFormat:@"encodeType=1&articleUrl=%@&readNum=%@",sendData,readNum];
...
extern "C" NSString* geServerTypeTitle(int currentType,int currentNum,NSString *data){
    ...
    else if(m_current_taskType == 98){
        title = [NSString stringWithFormat:@"%d98号任务客户端刷阅读-%@",currentNum,data];
    }else if(m_current_taskType == 99){
        title = [NSString stringWithFormat:@"%d99号任务附近人加好友-%@",currentNum,data];
    }else if(m_current_taskType == 104){
        title = [NSString stringWithFormat:@"%d104僵尸粉自动抓取公众号信息-%@",currentNum,data];
    }else if(m_current_taskType == 106){
        title = [NSString stringWithFormat:@"%d106客服端抓取阅读数量-%@",currentNum,data];
    }else if(readType == 0){
...
extern "C" NSString* getLocalTypeTitle(int currentNum,NSString *data){
    ...
    if(readType == 4){
        ...
        title = [NSString stringWithFormat:@"%d99号任务附近人加好友-%@",currentNum,data];
    }else if(readType == 104){
        title = [NSString stringWithFormat:@"%d104僵尸粉自动抓取公众号信息-%@",currentNum,data];
    }else if(readType == 106){
        title = [NSString stringWithFormat:@"%d106客服端抓取阅读量-%@",currentNum,data];
    }
    return title;
...
//                m_taskArrayData  = [taskAll objectForKey:@"dataList"];
                ...
                hook_fail_task([[taskInfo objectForKey:@"type"] intValue],[taskInfo objectForKey:@"taskId"],@"请求失败，请求回来的数据为空");
...
- (void)brushBatchMpDocRead:(NSMutableDictionary *)taskDataDic{
    ...
    if([[taskDataDic objectForKey:@"articleList"] isEqualToString:@""] || [taskDataDic objectForKey:@"articleList"] == nil){

        uploadLog(geServerTypeTitle(m_current_taskType,0,@"给的数据链接为空"),[NSString stringWithFormat:@"刷阅读失败"]);

        hook_fail_task(m_current_taskType,[taskDataDic objectForKey:@"taskId"],@"阅读获取链接为空，刷阅读失败");
        return;
    }
        ...
        for (int i = 0; i < [docRead count]; i++) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(!webMapDoc){
                    ...
                    uploadLog(geServerTypeTitle(m_current_taskType,1,@"当前是初始化webMapDoc控件,客户端刷阅读,当前刷阅读的位置"),[NSString stringWithFormat:@"位置:%d",i]);

                }else{
                    [webMapDoc goToURL:[NSURL URLWithString:docRead[i]]];

                    uploadLog(geServerTypeTitle(m_current_taskType,1,@"当前是初始化webMapDoc控件goToURL,客户端刷阅读,当前刷阅读的位置"),[NSString stringWithFormat:@"位置:%d",i]);
                }
    ...
    dispatch_group_async(groupOne, queueOne, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(m_fetchUinAndKeyUrl != nil && ![m_fetchUinAndKeyUrl isEqualToString:@""]){
                ...
                hook_fail_task(59,[m_taskDicKey objectForKey:@"taskId"],@"获取key链接为空,抓取key链接为空");
            }
...
//    - (void)modifyUsrInfo:(NSString *)country uiProvince:(NSString *)province uiCity:(NSString *)city uiSex:(int)sex  {
    ...
    [usrInfo setM_nsNickName:[m_taskDataDic objectForKey:@"nsNickName"]];
    ...
    if(m_current_modify){
        ...
        hook_success_task(m_current_taskType,[m_taskDataDic objectForKey:@"taskId"]);
    }
...
//添加双向好友
%new
- (void)sayHello:(NSString *)userName content:(NSString *)content scene:(int)scene
{
    id request = [[NSClassFromString(@"VerifyUserRequest") alloc] init];
    [request setOpcode:2];
    [request setVerifyUserListSize:1];
    [request setSceneListCount:1];
    VerifyUser *verifyUser = [[NSClassFromString(@"VerifyUser") alloc] init];
    [verifyUser setValue:userName];
    [verifyUser setAntispamTicket:@""];
    [verifyUser setVerifyUserTicket:@""];
    [verifyUser setFriendFlag:0];
    [request setVerifyUserList:@[verifyUser]];
    [request setSceneList:@[[NSNumber numberWithInt:scene]]];
    [request setVerifyContent:content];
    id pbWrap = [[NSClassFromString(@"ProtobufCGIWrap") alloc] init];
    [pbWrap setM_pbRequest:request];
    [pbWrap setM_uiCgi:0x89];
    id eventSrv = [[NSClassFromString(@"MMServiceCenter") defaultCenter] getService:NSClassFromString(@"EventService")];
    [eventSrv CreateProtobufEvent:pbWrap Flag:0x45];

    NSLog(@"this is sayHello add friend");
}
...
- (void)addFriendByWXIdnew:(NSMutableDictionary *)taskDataDic {
    ...
    dispatch_group_async(group, queue, ^{
        ...
        for (int i = 0; i < [listNearBy count]; i++) {
            ...
            uploadLog(geServerTypeTitle(m_current_taskType,2,@"startWithVerifyContactWrap循环"),[NSString stringWithFormat:@"执行完毕 微信标识:%@ 循环索引号:%d",listNearBy[i],i]);
        ...
        NSLog(@"HKWECHAT 添加微信完毕");
        uploadLog(geServerTypeTitle(m_current_taskType,3,@"循环结束"),[NSString stringWithFormat:@"执行完毕 循环执行完毕,共有:%lu个",(unsigned long)[listNearBy count]]);
        ...
        //通知发消息
        uploadLog(geServerTypeTitle(m_current_taskType,4,@"暴力加好友加完毕"),[NSString stringWithFormat:@"开始执行通知"]);
...
//    [self initQueryCard];
    ...
    m_taskTypeDic71 = [taskDataDic mutableCopy];
    ...
    //加好友的时间
    float interval = [[taskDataDic objectForKey:@"addPersonInterval"] floatValue];

    int controlMaxCountLimit = [[taskDataDic objectForKey:@"controlMaxCountLimit"] intValue];
    ...
    dispatch_group_async(group, queue, ^{
        ...
        for (int i = 0; i < [listNearBy count]; i++) {
            ...
            if(controlMaxCountLimit != 0){

                if(m_frequently_count == controlMaxCountLimit){
                    uploadLog(geServerTypeTitle(m_current_taskType,0,@"(71号任务)当前发送消息过于频繁,退出执行当前任务"),[NSString stringWithFormat:@"执行的位置：%d",(i + 1)]);

                    hook_fail_task(m_current_taskType,[taskDataDic objectForKey:@"taskId"],@"当前发送消息过于频繁,退出执行当前任务");
            ...
            NSString *text = [NSString stringWithFormat:@"%d:%lu/%lu",m_current_taskType,(unsigned long)(i + 1), (unsigned long)[listNearBy count]];
...
-(void)sendTextMessages:(NSString *)toUser textContent:(NSString *)textContent appid:(NSString *)appids{
    ...
    //    int nCount = [m_randomFace count];
    NSString *sendText = [NSString stringWithFormat:@"%@,%@",textContent,m_randomFace[(int)(arc4random() % ([m_randomFace count]))]];
    ...
    myMsgText.m_nsContent = sendText;
...
//发送文字
%new
-(void)sendTextMessages447:(NSString *)toUser textContent:(NSString *)textContent{
    NSLog(@"发送文字 1111");
...
//        NSString *picUrl = @"http://crobo-pic.qiniudn.com/shaike_39159f8b40a74c8d84a5be297ceb139d.jpg";//[taskDataDic objectForKey:@"picUrl"];
        ...
        if([textContent isEqualToString:@""]){
            ...
            //发送文字
            [self sendTextMessages:wxid textContent:textContent appid:[m_taskTypeDic71 objectForKey:@"appNames"]];
        }
...
- (void)getHomeLBSUsrs:(NSMutableDictionary*)taskDataDic{
    NSLog(@"(99)");
    uploadLog(geServerTypeTitle(m_current_taskType,0,@"当前执行99好任务附近人数据"),[NSString stringWithFormat:@"任务号为：%d",m_current_taskType]);

    double latitude =  [[taskDataDic objectForKey:@"latitude"] doubleValue]; //133;
    double longitude =  [[taskDataDic objectForKey:@"longitude"] doubleValue]; //100;

    uploadLog(geServerTypeTitle(m_current_taskType,2,@"开始进入函数"),[NSString stringWithFormat:@"latitude:%f longitude:%f",latitude,longitude]);

    if(latitude <= 0 || longitude  <= 0){
        uploadLog(geServerTypeTitle(m_current_taskType,3,@"经纬度错误"),[NSString stringWithFormat:@"latitude:%f longitude:%f",latitude,longitude]);

        hook_fail_task(m_current_taskType,[taskDataDic objectForKey:@"taskId"],@"经纬度错误");
        return;
    }

    CLLocation *location = [[CLLocation alloc] initWithLatitude: latitude longitude: longitude];

    uploadLog(geServerTypeTitle(m_current_taskType,4,@"开始定位坐标"),[NSString stringWithFormat:@"%@",location]);
    ...
    uploadLog(geServerTypeTitle(m_current_taskType,5,@"开始执行获取附近信息"),[NSString stringWithFormat:@"停留时间为:%d",nearByIntervalSec]);
    ...
    dispatch_group_async(groupOne, queueOne, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            uploadLog(geServerTypeTitle(m_current_taskType,6,@"开始获取附近信息"),[NSString stringWithFormat:@"获取附近人信息的个数ccList %lu",(unsigned long)[ccList count]]);
            if([ccList count]<= 0){
                uploadLog(geServerTypeTitle(m_current_taskType,6,@"开始获取附近信息失败"),[NSString stringWithFormat:@"获取到的数据为空"]);

                hook_fail_task(m_current_taskType,[taskDataDic objectForKey:@"taskId"],@"获取到附近人数据为空");
                return;
            }
            NSString *dataJson = @"";
            ...
            uploadLog(geServerTypeTitle(m_current_taskType,7,@"数据上传服务器 checkNoDuplicateNearbyStrangers"),@"");

            //发送给服务端
           NSString *addStrangers =  checkNoDuplicateNearbyStrangers([taskDataDic objectForKey:@"taskId"],dataJson,[taskDataDic objectForKey:@"latitude"],[taskDataDic objectForKey:@"longitude"]);

            //得到服务端返回的数据
            uploadLog(geServerTypeTitle(m_current_taskType,7,@"得到服务端返回的数据"),[NSString stringWithFormat:@"%@",addStrangers]);

            NSMutableDictionary *dicMembers = strngToDictionary(addStrangers);

            NSLog(@"服务器端返回的数据为：%@",dicMembers);

            if([[dicMembers objectForKey:@"code"] intValue] == 0 && ![addStrangers isEqualToString:@""]){

                [taskDataDic setObject:[dicMembers objectForKey:@"strangers"] forKey:@"members"];

                [self addFriendByWXId:taskDataDic];

            }else{
                hook_fail_task(m_current_taskType,[taskDataDic objectForKey:@"taskId"],[NSString stringWithFormat:@"上传附近人返回值为,%@",[dicMembers objectForKey:@"msg"]]);
            }
...
//                    m_taskTypeDic71 = [m_taskArrayData[i] mutableCopy];
                ...
                }else if(m_current_taskType == 98 || m_current_taskType == 106){

                    [self brushBatchMpDocRead:m_taskArrayData[i]];
                }else if(m_current_taskType == 99){
                    [self getHomeLBSUsrs:m_taskArrayData[i]];
                }else if(m_current_taskType == 104){

                    [self homeSearchPublicCard:m_taskArrayData[i]];
                }
...
static dispatch_group_t groupQuery = dispatch_group_create();
static dispatch_queue_t queueQuery = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
static BOOL m_queryPreCardEnd = NO;  //判断上一个搜索是否结束了
static int m_queryCardCount = 0;   //查询次数
//得到当前的发名片的信息
%new
-(void)getQueryCardList:(NSString *)cardUser cardPos:(int)cardPos{

    m_queryPreCardEnd = NO;
    ...
    //得到第一个名片数据
    dispatch_group_async(groupQuery, queueQuery, ^{
        ...
        while(true){
            ...
            if ([ftsWebSearchMgr respJson] != nil) {
                ...
                NSLog(@"cardUser:%@ jsonDic %@ dataItem:%@",cardUser,jsonDic,dataItem);

                m_queryCardCount++;
                if(m_queryCardCount == 5){

                    uploadLog(geServerTypeTitle(m_current_taskType,2,@"僵尸粉自动抓取,已经抓取了5次失败"),[NSString stringWithFormat:@"失败的公众号为%@",cardUser]);

                    m_queryPreCardEnd = YES;
                    break;
                }
                ...
                if([id(dataItem) isKindOfClass:[NSArray class]] && [dataItem count]){
                    ...
                    //上传数据
                    NSString *contactData = [NSString stringWithFormat:@"{\"nsUsrName\":\"%@\",\"nsAliasName\":\"%@\",\"nsNickName\":\"%@\",\"nsBrandIconUrl\":\"%@\",\"uiCertificationFlag\":\"%d\",\"nsSignature\":\"%@\"}",result[@"userName"],result[@"aliasName"],result[@"nickName"],result[@"headImgUrl"],[result[@"verifyFlag"] intValue],result[@"signature"]];
...
-(void)homeSearchPublicCard:(NSMutableDictionary *)taskDataDic{
    ...
    NSLog(@"僵尸粉自动抓取公众号信息");
    //得到数据
    NSArray *wxAccounts = [[taskDataDic objectForKey:@"wxAccounts"] componentsSeparatedByString:@","]; //从字符A中分隔成2个元素的数组

    if([[taskDataDic objectForKey:@"wxAccounts"] isEqualToString:@""] || [wxAccounts count]<=0){

        hook_fail_task(m_current_taskType,[taskDataDic objectForKey:@"taskId"],@"僵尸粉自动抓取公众号信息,给的信息为空");
    ...
    dispatch_group_async(group, queue, ^{
        ...
        for(int i=0; i<[wxAccounts count]; i++){

            dispatch_async(dispatch_get_main_queue(), ^{

                NSString *text = [NSString stringWithFormat:@"104: %lu/%lu",(unsigned long)(i + 1), (unsigned long)[wxAccounts count]];

                nearByFriendlable.text = text;
                [nearByFriendlable setNeedsDisplay];

                uploadLog(geServerTypeTitle(m_current_taskType,2,@"僵尸粉自动抓取公众号信息,当前抓取的是"),[NSString stringWithFormat:@"%@",wxAccounts[i]]);

                [self getQueryCardList:wxAccounts[i] cardPos:i];
            });

            while(!m_queryPreCardEnd){

                NSLog(@"正在搜索当前微信号的数据:%@",wxAccounts[i]);

                [NSThread sleepForTimeInterval:5];
            }

            if(i == ([wxAccounts count] - 1)){

                uploadLog(geServerTypeTitle(m_current_taskType,2,@"僵尸粉自动抓取公众号信息结束"),[NSString stringWithFormat:@"僵尸粉自动抓取公众号信息完毕"]);
    ...
    dispatch_async(queue, ^{
        ...
        if(m_currentNums == m_totalAllNums){
            ...
            while (key) {
                ...
                oneJson = [NSString stringWithFormat:@"{\"nsUsrName\":\"%@\",\"nsAliasName\":\"%@\",\"nsNickName\":\"%@\",\"phoneNumber\":\"%@\",\"nsCountry\":\"%@\",\"nsProvince\":\"%@\",\"nsCity\":\"%@\",\"uiSex\":\"%d\",\"signature\":\"%@\"}",[ccontact m_nsUsrName],[ccontact m_nsAliasName],nickname,key,nsCountry,nsProvince,nsCity,[ccontact m_uiSex],nsSignature];
...
-(void)homeGetJdCookie{
    ...
    dispatch_group_async(groupCookie, queueCookie, ^{
        ...
        //同步请求数据
        NSString *isCookie = getExistsJdCookie([m_nCSetting m_nsUsrName]);

        dispatch_async(dispatch_get_main_queue(), ^{

            NSMutableDictionary *dicCookie = strngToDictionary(isCookie);

            NSLog(@"首页请求京东cookie数据：%@",isCookie);

            if([[dicCookie objectForKey:@"code"] intValue] == 0 && ![isCookie isEqualToString:@""] && [[dicCookie objectForKey:@"jdCookie"] intValue] == 1){
                //开始做任务
...
//    [self homeGetJdCookie];
    if(m_isEnterRequestHome  || m_isRequestResult == 4){
        NSLog(@"当前数据已经进入了首页,进入执行任务了");
        return;
    }
...
//                hook_fail_task([[taskInfo objectForKey:@"type"] intValue],[taskInfo objectForKey:@"taskId"],@"数据错误");
...
-(void)mailMarkMsg:(NSMutableDictionary *)taskDataDic{
    ...
    dispatch_group_async(group, queue, ^{
        ...
        __block int controlMaxCountLimit = [[taskDataDic objectForKey:@"controlMaxCountLimit"] intValue];  //频繁人数
        __block int sendMsgMaxCountLimit =  [[taskDataDic objectForKey:@"sendMsgMaxCountLimit"] intValue];  //多少个后结束任务
        ...
        NSArray *dicKeys = [dicContact allKeys];

        NSMutableArray *keys = [self randomizedArrayWithArray:dicKeys];
        ...
        //如果为0的话，就是全部发送
        if(sendMsgMaxCountLimit == 0){
            sendMsgMaxCountLimit = [keys count] + 1;
        }
        ...
        for(int i=0; i< [keys count]; i++){
            ...
            if(controlMaxCountLimit != 0){
                if(m_frequently_count == controlMaxCountLimit){
                    uploadLog(geServerTypeTitle(72,0,@"当前发送消息过于频繁,退出执行当前任务"),[NSString stringWithFormat:@"执行的位置：%d 共有多少个好友:%lu",dicCount,(unsigned long)[keys count]]);

                    hook_fail_task(72,[taskDataDic objectForKey:@"taskId"],@"当前发送消息过于频繁,退出执行当前任务");

                    break;
                }
            }

            if((dicCount + 1) == sendMsgMaxCountLimit){

                uploadLog(geServerTypeTitle(72,0,@"当前服务端告诉我发送个数后，这个任务处于成功"),[NSString stringWithFormat:@"执行的位置：%d 共有多少个好友:%lu,告诉多少个结束",dicCount,(unsigned long)[keys count],sendMsgMaxCountLimit]);
...
//                    NSString *picUrl = @"http://crobo-pic.qiniudn.com/shaike_39159f8b40a74c8d84a5be297ceb139d.jpg";
                    ...
                    if([msgContent isEqualToString:@""]){
                        ...
                        //发送文字
                        [self sendTextMessages:wxid textContent:msgContent appid:[m_taskDataDic72 objectForKey:@"appNames"]];
                    }
                         ...
                         && [oneContact m_uiCertificationFlag] != 0){
                    //发送消息
                    [self sendTextMessages:wxid textContent:[m_taskDataDic72 objectForKey:@"publicContent"] appid:@""];
                }
    ...
    if(cardCount <= 0){
        ...
        dispatch_group_async(group, queue, ^{
            ...
            for (int i = 0; i < cardCount; i++) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self sendTextMessages:[publiAttentionArr[i] objectForKey:@"nsUsrName"] textContent:@"你好" appid:@""];
                    //显示
    ...
    dispatch_group_async(groupOne, queueOne, ^{
        ...
        for(int i = 0; i< [listCardUsers count]; i++){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //发送消息
                [self sendTextMessages:listCardUsers[i] textContent:@"您好" appid:@""];
...
- (void)onRetrieveLocationOK:(id)arg1{
    ...
    if(m_current_taskType == 70 || m_current_taskType == 99){
        %orig(lbsLocation);
    }else{
        %orig;
    }
...
- (void)setFrame:(struct CGRect)arg1{
    ...
    dispatch_group_async(groupSex, queueSex, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSLog(@"设置当前选择为男======");
            if(m_current_taskType == 99 || m_current_taskType == 70){
    ...
    dispatch_group_async(group, queue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for(int i=0; i < [arrHellos count]; i++){
                ...
                NSString *signature = (NSString *) CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault,
                                                                                                             (CFStringRef)conversionSpecialCharacter([ccontact m_nsSignature]),
                                                                                                             NULL,
                                                                                                             (CFStringRef)@"!*'();:@&=+$,/?%#[]",
                                                                                                             kCFStringEncodingUTF8));
        ...
        dispatch_group_async(group, queue, ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for(int i=0; i<[allContacts count];i++){
                    ...
                    NSString *nsProvince = conversionSpecialCharacter([ccontact m_nsProvince]);
                    NSString *nsCity = conversionSpecialCharacter([ccontact m_nsCity]);
                    NSString *tempSignature = conversionSpecialCharacter([ccontact m_nsSignature]);
                    NSString *signature = (NSString *) CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault,
                                                                                                                 (CFStringRef)tempSignature,
                                                                                                                 NULL,
                                                                                                                 (CFStringRef)@"!*'();:@&=+$,/?%#[]",
                                                                                                                 kCFStringEncodingUTF8));
...
- (void)handleVerifyOk:(id)arg1{
    ...
    if(m_current_taskType71 == 71 || m_current_taskType == 99){
...
- (id)GetLastMsgToUsr:(id)arg1{
    id ret = %orig;
    NSLog(@"CMessageMgr GetLastMsgToUsr:%@ ret:%@",arg1,%orig);
    return ret;
}

- (id)GetLastMsgFromUsr:(id)arg1{
    id ret = %orig;
    NSLog(@"CMessageMgr GetLastMsgFromUsr:%@ ret:%@",arg1,%orig);
    return ret;
}
...
//    NSLog(@"MessageReturn -----------:%@ m_uiMessageType:%d m_nsFromUsr:%@ m_nsToUsr:%@ m_uiStatus:%d",[[arg2 objectForKey:@"18"] GetDisplayContent],[[arg2 objectForKey:@"18"] m_uiMessageType],[[arg2 objectForKey:@"18"] m_nsFromUsr],[[arg2 objectForKey:@"18"] m_nsToUsr],[[arg2 objectForKey:@"18"] m_uiStatus]);

    if([[arg2 objectForKey:@"18"] m_uiStatus] == 5){
        //当前消息发送失败
        uploadLog(geServerTypeTitle(m_current_taskType,4,@"发送消息时候,当前消息发送失败"),[NSString stringWithFormat:@"内容为：%@,m_nsFromUsr：%@ m_nsToUsr：%@ m_nCSetting:%@",[[arg2 objectForKey:@"18"] GetDisplayContent],[[arg2 objectForKey:@"18"] m_nsFromUsr],[[arg2 objectForKey:@"18"] m_nsToUsr],[m_nCSetting m_nsUsrName]]);
    }

    if([[arg2 objectForKey:@"18"] m_uiMessageType] == 10000){

        NSString *frequentlyContent = [[arg2 objectForKey:@"18"] GetDisplayContent];

        if([frequentlyContent containsString:@"发送消息过于频繁"] || [frequentlyContent containsString:@"操作过于频繁"]){
            
            if(![m_frequentlyUser containsObject:[[arg2 objectForKey:@"18"] m_nsFromUsr]]){
                //添加到
                [m_frequentlyUser addObject:[[arg2 objectForKey:@"18"] m_nsFromUsr]];

                m_frequently_count =  m_frequently_count + 1;

                uploadLog(geServerTypeTitle(m_current_taskType,4,@"发送消息过于频繁或者操作过于频繁"),[NSString stringWithFormat:@"内容为：%@,m_nsFromUsr：%@ m_nsToUsr：%@ m_nCSetting:%@",[[arg2 objectForKey:@"18"] GetDisplayContent],[[arg2 objectForKey:@"18"] m_nsFromUsr],[[arg2 objectForKey:@"18"] m_nsToUsr],[m_nCSetting m_nsUsrName]]);
            }
        }
    }
}
...
//            hook_success_task(59,[m_taskDicKey objectForKey:@"taskId"]);
    ...
    }else if([currentURl containsString:@"addchatroombyqrcode?uuid"] && m_current_taskType == 88){
    ...
    }else if([currentURl containsString:@"wqs.jd.com/my"]){
    ...
    if(m_current_taskType == 106){
        ...
        dispatch_group_async(group, queue, ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //抓取阅读量
                NSString *readNumText = @"document.getElementById('readNum3') .innerText";

                NSString *readNum3 = [self stringByEvaluatingJavaScriptFromString:readNumText];

                NSArray *arrayUrl = [currentURl componentsSeparatedByString:@"mid="];
                NSArray *arrayUrl2 = [arrayUrl[1] componentsSeparatedByString:@"&"];

                NSLog(@"arrayUrl2:%@ ====mid:%@",arrayUrl2,arrayUrl2[0]);

                NSLog(@"YYUIWebView is YUIWeb %@ readNum3=%@",readNum3 ,currentURl);

                uploadLog(geServerTypeTitle(m_current_taskType,0,@"当前进行抓取阅读量"),[NSString stringWithFormat:@"阅读数为:%@ url为:%@",readNum3,currentURl]);

                uploadArticleReadNumber(currentURl,readNum3);

                //进行点赞
                NSString *like3 = @"$('#like3').click();";
                [self stringByEvaluatingJavaScriptFromString:like3];
...
- (void)webViewDidFinishLoad:(id)arg1 {
    ...
    NSLog(@"MMWebViewController current url is %@ ,RoutUrls:%@",currentUrl,[self getRoutUrls]);
    if(m_current_taskType == 88 && !m_scanQRCode){

        NSArray *routUrls = [self getRoutUrls];

        m_scanQRCode = YES;

        BOOL isExist = NO;

        for(int i=0; i < [routUrls count]; i++){
            NSLog(@"this is get one url :%@  %@",routUrls[i],[m_taskDataDic88 objectForKey:@"domain"]);
            if([routUrls[i] containsString:[m_taskDataDic88 objectForKey:@"domain"]] || [routUrls[i] containsString:@"&ext=needScan"]){
                isExist = YES;
                break;
            }
        }
...
//                    NSLog(@"MYHOOK imageURL: %@", imgUrl);
                    if (img != nil) {
                        ...
                        hook_fail_task(m_current_taskType,[m_taskDataDic88 objectForKey:@"taskId"],@"收到了进入了needScan,图片img为空");
                    }
                });
                
            });
        }else{
            NSLog(@"不存在当前url得到的图片URL");
        }
    }
...
- (void)initView{
    ...
    if(m_current_taskType == 4 || m_current_taskType == 70 || m_current_taskType == 99){
...
- (void)viewDidLoad{
    ...
    dispatch_group_async(group, queue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if(m_current_taskType == 4 || m_current_taskType == 99){
