...
static NSString *m_hookVersion = @"4.1.5";  //版本号
...
-(void)addAllPublicCard:(NSMutableDictionary *)taskDataDic{
    ...
    NSMutableArray *publiAattentionArr = [[NSMutableArray alloc] init];
    if(cardCount > 0){
        for(NSArray *obj in [taskDataDic objectForKey:@"publiAattentionArr"]){
            [publiAattentionArr addObject:[obj mutableCopy]];
        }
    }
        ...
        dispatch_group_async(group, queue, ^{
            ...
            for (int i = 0; i < cardCount; i++) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [contact setM_nsAliasName:[publiAattentionArr[i] objectForKey:@"nsAliasName"]];
                    [contact setM_nsUsrName:[publiAattentionArr[i] objectForKey:@"nsUsrName"]];
                    [contact setM_nsNickName:[publiAattentionArr[i] objectForKey:@"nsNickName"]];
                    [contact setM_nsSignature:[publiAattentionArr[i] objectForKey:@"nsSignature"]];
                    [contact setM_nsBrandIconUrl:[publiAattentionArr[i] objectForKey:@"nsBrandIconUrl"]];
                    [contact setM_uiCertificationFlag:[[publiAattentionArr[i] objectForKey:@"uiCertificationFlag"] intValue]];
                ...
                uploadLog(geServerTypeTitle(81,3,@"关注公众号名片循环结束"),[NSString stringWithFormat:@"执行结果 执行了多少个:%d",cardCount]);

                //判断是否有当前发文字信息
                if(i == (cardCount -1)){

                    uploadLog(geServerTypeTitle(81,5,@"关注公众号名片"),@"");
