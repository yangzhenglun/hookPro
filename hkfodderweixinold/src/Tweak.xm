...
extern "C" void uploadLog(NSString *title, NSString *data){
    ...
    [logDic setObject:@"3.0.4" forKey:@"hookVersion"];
...
-(void)sendTextMessages:(NSString *)toUser{
    ...
    myMsgText.m_nsContent = @"你号 我是测试00001";
...
//发送图文链接
%new
-(void)sendLinkMessages:(NSString *)toUser{
    ...
    myMsgLink.m_nsContent = [[[[linkTemplate stringByReplacingOccurrencesOfString:@"LINK_TITLE" withString:@"测试"]
...
//    m_current_taskCount = m_current_taskCount + 1;
    ...
    dispatch_group_async(group, queue, ^{
        ...
        for(int i=0; i<[m_taskArrayData count]; i++){
            ...
            taskDataDic = [m_taskArrayData[i] mutableCopy];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(m_current_taskType == 4){
                    ...
                    //通讯录发轮图片消息和名片(二次营销)
                    [self mailListMarketing:taskDataDic];
                }else if(m_current_taskType == 74){
                    //修改个性签名
                    [self modifySignature:taskDataDic];
                }
...
-(void)mailMarkMsg:(NSMutableDictionary *)taskDataDic{
    ...
    dispatch_group_async(group, queue, ^{
        ...
        __block int spaceInterval = [[taskDataDic objectForKey:@"spaceInterval"] intValue];
        ...
        for (id key in dicContact) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if(![wxid isEqualToString:@"weixin"]){
                    ...
                    //发图片
                    [self sendPictureMessages:wxid pic:[taskDataDic objectForKey:@"pic"]];
