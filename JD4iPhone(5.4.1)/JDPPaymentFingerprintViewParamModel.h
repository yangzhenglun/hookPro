//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSString, UINavigationController;

@interface JDPPaymentFingerprintViewParamModel : NSObject
{
    _Bool _needAutoValidate;
    NSString *_pin;
    NSString *_bizType;
    NSString *_bizId;
    NSString *_accountParam;
    UINavigationController *_navigationController;
}

@property(nonatomic) _Bool needAutoValidate; // @synthesize needAutoValidate=_needAutoValidate;
@property(retain, nonatomic) UINavigationController *navigationController; // @synthesize navigationController=_navigationController;
@property(copy, nonatomic) NSString *accountParam; // @synthesize accountParam=_accountParam;
@property(copy, nonatomic) NSString *bizId; // @synthesize bizId=_bizId;
@property(copy, nonatomic) NSString *bizType; // @synthesize bizType=_bizType;
@property(copy, nonatomic) NSString *pin; // @synthesize pin=_pin;
- (void).cxx_destruct;

@end

