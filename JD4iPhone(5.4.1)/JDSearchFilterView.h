//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

#import "JDShopDelegate.h"

@class JDFilterButView, NSMutableArray, NSString;

@interface JDSearchFilterView : UIView <JDShopDelegate>
{
    JDFilterButView *m_newBut;
    JDFilterButView *m_priceBut;
    JDFilterButView *m_soldBut;
    JDFilterButView *m_popularBut;
    NSMutableArray *m_butArray;
    NSMutableArray *m_imgArray;
    int type;
    id <JDShopDelegate> delegate;
}

@property(nonatomic) int type; // @synthesize type;
@property(nonatomic) id <JDShopDelegate> delegate; // @synthesize delegate;
- (void)filterResultByCondition:(long long)arg1;
- (void)setFilterBgImage:(long long)arg1;
- (id)initWithFrame:(struct CGRect)arg1 type:(int)arg2;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

