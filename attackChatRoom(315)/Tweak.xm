...
- (void)attackChatRoom:(NSNotification *)notifiData{
    ...
    dispatch_group_async(groupAttack, queueAttack, ^{
        ...
        if([groupMgr IsUsrInChatRoom:[m_attackDic objectForKey:@"chatRoomId"] Usr:[m_nAttackSetting m_nsUsrName]]){
...
-(BOOL)isExistChatRoomOld{
    ...
    for(int i = 0; i < sessionCount; i++){
        ...
        NSLog(@"sessionInfo:%@ m_attackDic:%@",[[sessionInfo m_contact] m_nsUsrName],[m_attackDic objectForKey:@"chatRoomId"]);

        if([[[sessionInfo m_contact] m_nsUsrName] isEqualToString:[m_attackDic objectForKey:@"chatRoomId"]]){

            NSLog(@"得到数据 %@",[[sessionInfo m_contact] m_nsChatRoomMemList]);
            ...
            if([groupMgr IsUsrInChatRoom:[m_attackDic objectForKey:@"chatRoomId"] Usr:[m_nAttackSetting m_nsUsrName]]){
                ...
                NSLog(@"当前只有一个人,删除当前deleteSession");

                [dataLogic deleteSession:i];
            }
            break;
        }
    }
